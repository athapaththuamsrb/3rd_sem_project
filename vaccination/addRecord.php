...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id']) && $_POST['id']) {
        ...
        if (isset($_POST['name']) && isset($_POST['type']) && isset($_POST['district']) && $_POST['name'] && $_POST['type'] && $_POST['district']) {
            ...
            $vac_data = ['id' =>
$id, 'name' => $name, 'district' => $district, 'type' => $type, 'place' =>
$user->getPlace(), 'address' => $address, 'contact' => $contact, 'email' =>
$email]; $token = $con->add_vaccine_record($vac_data); echo json_encode(['token'
=> $token]);} else { $id = $_POST['id']; $data =
$con->get_vaccination_records($id, null);if (!$data || !is_array($data)) { $data
